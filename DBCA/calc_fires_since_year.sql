CREATE TABLE :table AS (
    ...
    FROM (
        SELECT 
            poly_id, 
            (SELECT count(DISTINCT thisfireseason) FILTER(WHERE thisfireseason - prevfireseason > 1) AS numfires)
        FROM 
            ids
        GROUP BY poly_id
    ) sub
